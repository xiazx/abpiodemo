...
namespace Abp.Demo.Shared.Dto
{
    ...
    public class SortedKeyValueData
    {
        ...
        {
            return new SortedDictionary<string, string>(_values);
        }
    ...
    public static class SortedKeyValueDataExtensions
    {
        ...
        /// <param name="encode">参数是否进行Url编码</param>
        public static string ToUrlParams(this SortedKeyValueData data, bool encode = true)
        {
            ...
            if (data != null)
            {
                foreach (var item in data.AsEnumerable())
                {
                    ...
                    if (encode)
                        query.AppendFormat("{0}={1}", HttpUtility.UrlEncode(item.Key), HttpUtility.UrlEncode(item.Value));
                    else
        ...
        /// </summary>
        /// <param name="data"></param>
        /// <param name="url">要拼接的地址</param>
        public static string ToUrlParams(this SortedKeyValueData data, string url)
        {
            var queryString = data.ToUrlParams(true);
            if (string.IsNullOrEmpty(url)) return queryString;

            return string.Concat(url, url.LastIndexOf("?") > 0 ? "&" : "?", queryString);
        }
